...
namespace BE.Data.DataRoles
{
    ...
    {
        List<Permission_Use_Menu> RoleSample(int idUser, int idGroup, int idUserCreated = 0);
    }

    public class DataSample : IDataSample
    {
        private readonly AppDbContext _db;
        public DataSample(AppDbContext db)
        {
            _db = db;
        }

        public List<Permission_Use_Menu> RoleSample(int idUser, int idGroup, int idUserCreated = 0)
        {
            ...
            foreach (var item in modules)
            {
                ...
                if (item.nameModule.ToLower().Equals("leaveOffs"))
                {
                ...
                {
                    permissonUser = new Permission_Use_Menu(item.idModule, idUser, 0, 0, 1, 0, 0, 0, 0, 0, 0, 1, idUserCreated);
                }
                else if (item.nameModule.ToLower().Equals("paids"))
                {
                    permissonUser = new Permission_Use_Menu(item.idModule, idUser, 0, 1, 1, 1, 1, 1, 1, 0, 0, 1, idUserCreated);
                }
