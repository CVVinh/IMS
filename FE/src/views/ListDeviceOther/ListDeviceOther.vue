    ...
    <LayoutDefaultDynamic>
        ...
        <div class="mx-3 mt-3">
            <DataTable
                ...
                :globalFilterFields="[
                    '#', 'branchName', 'deviceName', 'lastName', 'firstName', 'orders1.idOrder', 'orders1.idBranch',
                    'orders1.idDevice', 'orders1.amount', 'orders1.userUpdated', 'orders1.dateUpdated',
                    'orders1.userCreated', 'orders1.dateCreated', 'orders1.isDelete', 'orders1.note',
                    'orders1.statusOrder', 'orders1.statusDevice']">
                <template #header>
                    <div class="flex justify-content-center">
                        <h5 class="" style="color: white;">Devices Order List</h5>
                        <div class="inline">
                            <Export label="Export" @click="exportCSV($event)" style='margin-right: 10px;' />
                            <Add @click="openDialog()" label="Add" />
                ...
                <Column field="orders1.idOrder" header="Order" sortable>
                    <template #body="{ data }">
                        {{ data.orders1.idOrder }}
                    </template>
                </Column>
                <Column field="branchName" header="Branch Name" sortable style="min-width: 10rem">
                    <template #body="{ data }">
                        {{ data.branchName }}
                    </template>
                </Column>
                <Column field="deviceName" header="Device Name" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                        {{ data.deviceName }}
                    </template>
                </Column>
                <Column field="orders1.amount" header="Amount" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                        {{ data.orders1.amount }}
                    </template>
                </Column>
                <Column field="orders1.statusOrder" header="Status Order" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                        {{ data.orders1.statusOrder }}
                    </template>
                </Column>
                <Column field="orders1.statusDevice" header="Status Device" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                        {{ data.orders1.statusDevice }}
                    </template>
                </Column>
                <Column field="orders1.note" header="Note" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                        {{ data.orders1.note }}
                    </template>
                </Column>
...
export default {
    ...
    methods : {
        ...
        showError() {
            this.$toast.add({severity:'error', summary: 'Error Message', detail:'You refuse to delete', life: 3000});
        },
        ...
        getListBranch() {
            HTTP.get('Branchs/getListBranch').then((res) => {
                if (res.data) {
                    res.data.forEach(item => {
