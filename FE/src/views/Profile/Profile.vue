        ...
        <form @submit.prevent="handleSubmit(!v$.$invalid)" class="container">
            <div class="box">
                <div class="box-top" v-bind="{ backgroundImage: `url('../../assets/svg-profile.svg')` }">
                    ...
                    <h3 class="user-code">User name: {{ user.userCode }}</h3>
                    <h6 class="user-swd">Start work date: {{ renderDateStartWork() }}</h6>
                </div>
            </div>
            <div class="box box-bottom">
                <div class="row">
                    <div class="field">
                        <label for="firstName" :class="{ 'p-error': v$.user.firstName.$invalid && submitted }">First
                            Name</label>
                        <InputText id="firstName" type="text" v-model="v$.user.firstName.$model"
                    ...
                    <div class="field">
                        <label for="lastName" :class="{ 'p-error': v$.user.lastName.$invalid && submitted }">Last
                            Name</label>
                        <InputText id="lastName" type="text" v-model="v$.user.lastName.$model"
                ...
                <div class="row">
                    <div class="field">
                        <label for="birthday" :class="{ 'p-error': !checkDateOfBirth && submitted }">Birthday</label>
                        <Calendar v-if="!loading" id="birthday" v-model="v$.user.dOB.$model" :showIcon="true"
                            :class="{ 'calendar-error': !checkDateOfBirth }" />
                        <small v-if="!checkDateOfBirth && submitted" class="p-error small-error">Birthday smaller 60 and
                            bigger 18</small>
                    </div>
                    <div class="field">
                        <label for="identityCard"
                            :class="{ 'p-error': v$.user.identitycard.$invalid && submitted }">Identity Card</label>
                        <InputText id="identityCard" type="text" v-model="v$.user.identitycard.$model"
                        ...
                        " class="p-error">The number length must be 9 or 12</small>
                    </div>
                </div>
                <div class="row">
                    <div class="field">
                        <label for="phoneNumber" :class="{ 'p-error': v$.user.phoneNumber.$invalid && submitted }">Phone
                            Number</label>
                        <InputText id="phoneNumber" type="text" v-model="v$.user.phoneNumber.$model"
                        ...
                        <small
                            ...
                            class="p-error">Phone number length must be 10
                        </small>
                    </div>
                    <div class="field">
                        <label for="email" :class="{ 'p-error': v$.user.email.$invalid && submitted }">Email</label>
                        <InputText id="email" type="text" v-model="v$.user.email.$model"
                ...
                <div class="row">
                    <div class="field">
                        ...
                        <InputText id="skype" type="text" v-model="v$.user.skype.$model"
                    ...
                    <div class="field">
                        ...
                        <Dropdown id="group" type="text" v-model="v$.user.idGroup.$model" :options="groupList"
                            optionLabel="nameGroup" optionValue="id" :disabled="true" />
                    </div>
                </div>
                <div class="row">
                    <div class="field">
                        <label for="gender" :class="{ 'p-error': v$.user.gender.$invalid && submitted }">Gender</label>
                        <Dropdown id="gender" v-model="v$.user.gender.$model" :options="optionGender" optionValue="code"
                            optionLabel="name" :class="{ 'dropdown-error': v$.user.gender.$error }" v-bind:style="{
                                borderColor: v$.user.gender.$invalid && submitted ? errorColor : '#ced4da',
                            }" />
                        <small v-if="v$.user.gender.$invalid && submitted" class="p-error">Gender is required</small>
                    </div>
                    <div class="field">
                        <label for="maritalStatus">Marital Status</label>
                        <Dropdown id="maritalStatus" v-model="v$.user.maritalStatus.$model"
                            :options="optionMaritalStatus" optionValue="code" optionLabel="name" />
                    </div>
                </div>
                <div class="row">
                    <div class="field">
                    ...
                    <div class="field">
                        <label for="yearGraduated"
                            :class="{ 'p-error': v$.user.yearGraduated.$invalid && submitted }">Year Graduated</label>
                        <InputText id="yearGraduated" type="text" v-model="v$.user.yearGraduated.$model"
                ...
                <div class="row">
                    <div class="field">
                        ...
                        <InputText id="address" type="text" v-model="v$.user.address.$model" :showIcon="true"
                ...
                <div class="row">
                    <div class="bottom-button">
                        <Button class="p-button-outlined" @click="handleReset">Reset</Button>
            ...
            HTTP.get('Group/getListGroup/')
                ...
                .catch((er) => {
                    this.$toast.add({
                        ...
                        summary: 'Error ',
                        detail: "Can't get information of group!",
                        life: 3000,
                    })
                })
        },
            ...
            HTTP.get('Users/getUserById/' + user.Id)
                .then((res) => {
                    if (res.status == 200) {
                        let arr = res.data
                        arr = res.data
                        arr.dateStartWork = res.data.dateStartWork === null ? null : new Date(res.data.dateStartWork)
                ...
                .catch((er) => {
                    this.$toast.add({
                        severity: ToastSeverity.ERROR,
                        summary: 'Error ',
                        detail: "Can't get information of user!",
                        life: 3000,
                    })
                })
        },
