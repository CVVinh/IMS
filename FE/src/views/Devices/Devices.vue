<template>
    <LayoutDefaultDynamic>
        ...
        <div class='mx-3 mt-3'>
            <DataTable :value='arr' ref='devices' :paginator='true' class='p-datatable-customers' :rows='5'
                dataKey='idDevice' :rowHover='true' v-model:filters='filters' v-model:selection='selectedDevice'
                sortField='dateCreated' :sortOrder='-1' :loading='loading' responsiveLayout='scroll'
                paginatorTemplate='FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown'
                :rowsPerPageOptions='pageIndex'
                currentPageReportTemplate='Showing {first} to {last} of {totalRecords} entries'
                :globalFilterFields="['#', 'idDevice', 'deviceName', 'info', 'isDelete', 'userUpdated', 'dateUpdated', 'userCreated', 'dateCreated', 'note']">
                <template #header>
                    <div class='flex justify-content-center'>
                        <h5 class='' style='color: white;'>Devices List</h5>
                        <div class='inline'>
                ...
                <Column field='deviceName' header='Name Device' sortable>
                    <template #body='{ data }'>
                        {{ data.deviceName }}
                    </template>
                </Column>
                <Column field='info' header='Info' sortable style='min-width: 10rem'>
                    <template #body='{ data }'>
                        {{ data.info }}
                    </template>
                </Column>
                <Column field='note' header='Note' sortable style='min-width: 8rem'>
                    <template #body='{ data }'>
                        {{ data.note }}
                    </template>
                </Column>
                <Column header='&emsp;&emsp;&emsp;Action' style='min-width: 15rem'>
                    <template #body='{ data }'>
                        <Edit class='mazin' @click='openDialogEdit(data)'
                            :disabled='enableDeleteEdit(data.isDeleted)' />
                        <Delete class='maz' @click='confirmDelete(data.idDevice)'
                            :disabled='enableDeleteEdit(data.isDeleted)' />
                    </template>
                </Column>
            </DataTable>
        </div>
        <Dialog header='Access is denied!' :visible='displayBasic'
        ...
        <AddDevices :isOpenDialog='isOpenDialog' :deviceSelected='selectedEdit' @closeDialog='closeDialog'
    ...
    methods: {
        ...
        confirmDelete(id) {
            this.$confirm.require({
                message: 'Do you want to delete this devices?',
