            ...
            <div class='col-sm-3 card p-2'>
                <div class='card'>
                    <template v-if='userGroup.length <=0'>
                        <p class='p-2 text-black-50'> No User Group found.</p>
                    </template>
                    <OrderList v-model='userGroup' style='height: calc(100vh - 150px)' dataKey='id'
                    ...
                    <div v-if='userGroup.length > 0' class='col-12 d-flex p-lg-3 justify-content-end position-absolute bottom-0'>
                        <button label='CANCEL' class="btn btn-secondary me-2" @click="cancel">Cancel</button>
                        <button label='SAVE' class="btn btn-primary" @click="confirmSave()" :disabled="!this.selectedGroupId">Save</button>
                    </div>
                </div>
            </div>
                        ...
                        <div class='col-sm-9 tab-item-sub'>
                            <div class='col-12 d-flex'>
                                <div class='col-sm-2 field-checkbox d-flex align-items-center'>
                                    <input class='input-checkbox' type="checkbox" :id="module.id" :value="module.id"
                                        v-bind:checked='module.access' :disabled="!this.selectedGroupId"
                                        @change="isSelectedCheck(module.id, $event.target.checked)" />
                                    <label :for='module.id' class='ms-3'>Access</label>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async selectedGroup(data) {
            ...
            const listModuleAccess = await HTTP.get(ApiApplication.PERMISSION_GROUP_MENU.GET_BY_USER_GROUP + "?idGroup=" + this.selectedGroupId);
        ...
        getUserMenuModule() {
            ...
            if (this.selectedGroupId) {
                HTTP.get(ApiApplication.PERMISSION_USER_MENU.GET_USER_MENU + "?idUser=" +  this.selectedGroupId).then(res => {
                    if (res) {
                        this.listModule.map(module => {
                            res.data.forEach(item => {
                        ...
                        this.listModule.filter(item => {
                            if (item.id) {
                                this.arrModulePermissionAll.push([0, item.id, item.add??0, item.update??0, item.delete??0, item.export??0]);
                            }
                        })
                        this.loading = false;
                    } else {
        ...
        confirmSave() {
            if (this.arrModulePermissionAll.length > 0) {
                this.$confirm.require({
                    message: `The process can take a few minutes. Do you want to continue?`,
                    header: 'Confirmation',
        ...
        save() {
            if (this.selectedGroupId) {
                ...
                HTTP.post(ApiApplication.PERMISSION_GROUP_MENU.GET_PERMISSION_GROUP
                    + "?idGroup=" + this.selectedGroupId + "&idUserAdd="
                    + token.Id, this.selectedAccess
                ).then(res => {
                    if (res) {
        ...
        acceptPermissionUserMenu() {
            const token = LocalStorage.jwtDecodeToken()
            HTTP.post(ApiApplication.PERMISSION_USER_MENU.ADD_BY_GROUP
                + "?idGroup=" + this.selectedGroupId + "&idUserAdd="
                + token.Id, this.arrModulePermissionAll
            ).then(res => {
                if (res) {
                    this.$toast.add({
                        severity: 'success',
