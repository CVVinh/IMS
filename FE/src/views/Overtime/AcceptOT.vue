    ...
    <layout-default-dynamic>
        <div class="mx-5 mt-5">
            <DataTable :value="data" :paginator="true" class="p-datatable-customers" :rows="10" dataKey="id"
                :rowHover="true" v-model:filters="filters" v-model:selection="selectedOT" filterDisplay="menu"
                ...
                <template #header>
                    <div class="flex align-items-center">
                        ...
                        <div class="filter-pro">
                            <h5 style="display: inline">Project:&emsp;</h5>
                            <Dropdown class="p-column-filter" v-model="filters['idProject'].value" :options="projects"
                                optionLabel="name" optionValue="id" placeholder="Select a Project" />
                ...
                <template #empty> No OT or Remote found. </template>

                <!-- Body -->
                <Column selectionMode="multiple" headerStyle="width: 3rem"></Column>
                <Column field="date" header="Date" sortable dataType="date" style="min-width: 10 rem">
                    <template #body="{ data }">
                        {{ formatDate(data.date) }}
                    </template>
                </Column>
                <Column field="user" header="User OT" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                        {{ username[data.user] }}
                    </template>
                </Column>
                <Column field="leadCreate" header="Proposed Lead" sortable style="min-width: 11rem">
                    <template #body="{ data }">
                ...
                <Column field="realTime" header="OT Time" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                        {{ data.realTime }}
                    </template>
                </Column>
