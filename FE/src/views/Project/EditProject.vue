    ...
    <LayoutDefault>
        <div class="container mt-4">
            ...
            <form @submit.prevent="handleSubmit(!v$.$invalid)" class="p-fluid">
                <div class="field">
                    <label for="name" :class="{ 'p-error': v$.data.name.$invalid && submitted }">Tên dự án<span
                            style="color: red;">*</span></label>
                    <div class="p-float-label" :class="{ 'form-group--error': v$.data.name.$error }">
                        <InputText id="name" v-model="v$.data.name.$model"
                    ...
                    <small v-else-if="(v$.data.name.$invalid && submitted) || v$.data.name.$pending.$response"
                        class="p-error">{{
                        v$.data.name.required.$message.replace('Value', 'Tên dự án')
                        }}</small>
                </div>
                <div class="field">
                    <label for="projectCode" :class="{ 'p-error': v$.data.projectCode.$invalid && submitted }">Mã dự
                        án<span style="color: red;">*</span></label>
                    <div class="p-float-label" :class="{ 'form-group--error': v$.data.projectCode.$error }">
                        <InputText disabled="true" id="projectCode" v-model="v$.data.projectCode.$model"
                    ...
                    <small
                        v-else-if="(v$.data.projectCode.$invalid && submitted) || v$.data.projectCode.$pending.$response"
                        class="p-error">{{
                        v$.data.projectCode.required.$message.replace('Value', 'Mã dự án')
                        }}</small>
                </div>
                <div class="field">
                    <label for="description" :class="{ 'p-error': v$.data.description.$invalid && submitted }">Mô
                        tả</label>
                    <div class="p-float-label" :class="{ 'form-group--error': v$.data.description.$error }">
                        <Textarea id="description" style="height: 150px;" v-model="v$.data.description.$model"
                ...
                <div class="field">
                    <label for=" startDate"
                        :class="{ 'p-error': (v$.data.startDate.$invalid || !checkStartdate()) && submitted }">Ngày bắt
                        đầu<span style="color: red;">*</span></label>
                    <div class="p-float-label" :class="{ 'form-group--error': v$.data.startDate.$error }">
                        <Calendar id="startDate" v-model="v$.data.startDate.$model" :showIcon="true"
                            :class="{ 'p-invalid': (v$.data.startDate.$invalid || !checkStartdate()) && submitted }" />
                    </div>
                    <small v-if="(v$.data.startDate.$invalid && submitted) || v$.data.startDate.$pending.$response"
                        class="p-error">{{
                        v$.data.startDate.required.$message.replace('Value', 'Ngày bắt đầu')
                        }}</small>
                ...
                <div class="field">
                    <label for=" endDate"
                        :class="{ 'p-error': (v$.data.endDate.$invalid || !checkEnddate()) && submitted }">Ngày kết
                        thúc<span style="color: red;">*</span></label>
                    <div class="p-float-label" :class="{ 'form-group--error': v$.data.endDate.$error }">
                        <Calendar id="endDate" v-model="v$.data.endDate.$model" :showIcon="true"
                            :class="{ 'p-invalid': (v$.data.endDate.$invalid || !checkEnddate()) && submitted }" />
                    </div>
                    <small v-if="(v$.data.endDate.$invalid && submitted) || v$.data.endDate.$pending.$response"
                        class="p-error">{{
                        v$.data.endDate.required.$message.replace('Value', 'Ngày kết thúc')
                        }}</small>
                ...
                <div class="field">
                    <label for="userId" :class="{ 'p-error': v$.data.userId.$invalid && submitted }">Quản lý dự án<span
                            style="color: red;">*</span></label>
                    <div class="p-float-label" :class="{ 'form-group--error': v$.data.userId.$error }">
                        <Dropdown disabled="true" v-model="v$.data.userId.$model" :options="user" optionLabel="userCode"
                            optionValue="id" :class="{ 'p-invalid': v$.data.userId.$invalid && submitted }" />
                    </div>
                    <small v-if="(v$.data.userId.$invalid && submitted) || v$.data.userId.$pending.$response"
                        class="p-error">{{
                        v$.data.userId.required.$message.replace('Value', 'Quản lý dự án')
                        }}</small>
                </div>
                <div class="field">
                    <label for="leader" :class="{ 'p-error': v$.data.leader.$invalid && submitted }">Leader<span
                            style="color: red;">*</span></label>
                    <div class="p-float-label" :class="{ 'form-group--error': v$.data.leader.$error }">
                        <Dropdown v-model="v$.data.leader.$model" :options="leser" optionLabel="userCode"
                            optionValue="id" :class="{ 'p-invalid': v$.data.leader.$invalid && submitted }" />
                    </div>
                    <small v-if="(v$.data.leader.$invalid && submitted) || v$.data.leader.$pending.$response"
                        class="p-error">{{
                        v$.data.leader.required.$message.replace('Value', 'Leader')
                        }}</small>
                </div>
                <div class="">
...
export default {
    ...
    methods: {
        handleSubmit(isFormValid) {
            ...
            if (!this.valid || !this.checkStartdate() || !this.checkEnddate())
                return
            this.editData();
        },
        editData() {
            ...
            if (data) {
                ...
                HTTP.put('Project/updateProject', data).then(res => {
                    if (res.status == 200) {
                        this.showSuccess()
                        router.push('/project')
                    }
        ...
        showSuccess() {
            this.$toast.add({ severity: 'success', summary: 'Success Message', detail: 'Chỉnh sửa dự án thành công !!!', life: 3000 });
        },
        showWarn() {
            this.$toast.add({ severity: 'warn', summary: 'Warn Message', detail: 'Chỉnh sửa dự án thất bại !!!', life: 3000 });
        },
        checkStartdate() {
    ...
    mounted() {
        if (!UserRoleHelper.isPm() && !UserRoleHelper.isAdmin())
            router.push('/project')
