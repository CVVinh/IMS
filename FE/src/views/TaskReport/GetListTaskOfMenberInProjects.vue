    ...
    <LayoutDefaultDynamic>
        ...
        <div class="mx-4 mt-3">
            <Card style="width: 100%; margin-bottom: 2em; background-color: #d3d3d382">
                <template #content>
                    <DataTable :value="dataUsers" :rows="5" ref="dt" :paginator="true" :loading="loading"
                        ...
                        <template #header>
                            <div class="row">
                                <div class="col-md-5">
                                    <label for="Project_drop" class="me-2">Project</label>
                                    <Dropdown id="Project_drop" style="width: 60%"
                                        v-model="fillterProject.selectedProject" :options="dataProjects"
                                        optionLabel="name" :filter="true" placeholder="Select a Project"
                                        :showClear="true">
                                    ...
                                    <Button class="p-button-outlined p-button-secondary ms-2" @click="clearSelect()">
                                        <span class="pi pi-sync p-button-icon" style="font-size: 23px"></span></Button>
                                </div>
                                    ...
                                    <div class="d-flex">
                                        ...
                                        <div class="me-2">
                                            <Button class="p-button-outlined p-button-info"
                                                @click="handlerFillterProject" icon="pi pi-filter-fill" />
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-5">

                                    <label class="me-2" style="margin-top: 20px">Staff</label>
                                    <div class="input-text" style="display: contents">
                                        <span class="p-input-icon-left"
                                            style="margin-left: 17px;margin-top: 10px ; width: 60%; ">
                                            <InputText class="p-inputtext-sm" style="padding-left: 1rem; width: 100%;"
                                                v-model="searchtext" placeholder="Keyword Search" />
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <Column field="time_estimate" header="Time Estimate">
                            <template #body="{ data }"> {{ Math.round(data.time_estimate * 100) / 100 }}h </template>
                        </Column>
                        <Column field="total_time_spent" header="Total Time Spent">
            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="(item, index) in dataDetail" :key="index">
                        ...
                        <td class="text-center">{{ Math.round((item.time_stats.time_estimate / 3600) * 100) / 100 }}h
                        </td>
                        <td class="text-center">{{ Math.round((item.time_stats.total_time_spent / 3600) * 100) / 100 }}h
                        </td>
        ...
        searchtext: {
            handler: function Change(newText) {
                if (newText != '') {
                    ...
                    if (this.fillterProject.selectedProject != null || this.fillterProject.startDate != null && this.fillterProject.endDate != null) {
                        this.loading = true
                        this.dataUsers = []
                        this.dataIssue = []
                        this.handleAction()
